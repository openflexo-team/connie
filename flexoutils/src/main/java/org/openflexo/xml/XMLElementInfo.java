...
public class XMLElementInfo {
	...
	protected static final Logger LOGGER = Logger.getLogger(XMLElementInfo.class.getPackage().getName());
	...
	public XMLElementInfo addElement(String uri, String localName, String qName) {
		XMLElementInfo returned = new XMLElementInfo();
		returned.setName(localName);
		returned.setQName(qName);
		returned.setURI(uri);
		...
		if (qName != null && localName != null) {
			...
			if (!NSPrefix.isEmpty() && !uri.isEmpty())
				returned.addNamespace(NSPrefix, uri);
		}
	...
	public List<XMLElementInfo> getElements() {
		return elements;
	}
